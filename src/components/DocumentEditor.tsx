...
const DocumentEditor: React.FC<DocumentEditorProps> = ({
  ...
  }, [propDocument, loadLocalDocument, isAuthenticated, documentService, localDocumentId, tiptapContent, editorContent]);
  ...
  }, [localDocumentId, tiptapContent, editorContent, wordCount, charCount, isAuthenticated, documentService, onDocumentSaved]);

  const hasUnsavedChanges = useCallback((): boolean => {
    if (!currentDocument) return false;
    const currentContent = tiptapContent || editorContent;
    return currentContent !== currentDocument.content;
  }, [currentDocument, tiptapContent, editorContent]);
  ...
  const handleNewDocument = useCallback(() => {
    ...
    setTimeout(() => setAutoSaveStatus(''), 2000);
  }, [localDocumentId, saveToLocalStorage, isAuthenticated, documentService, onDocumentUpdate, tiptapContent, editorContent]);
  ...
  }, [isAuthenticated, currentDocument, documentService, selectedStorageOption, wordCount, charCount, saveToLocalStorage, tiptapContent, editorContent]);

  useEffect(() => {
    const interval = setInterval(() => {
      if (localDocumentId && unsavedChanges) {
        const content = tiptapContent || editorContent;
        if (content && content !== '<p>Start writing...</p>' && content !== '<p><br></p>') {
          saveToLocalStorage();
      ...
      if (isAuthenticated && localDocumentId && unsavedChanges) {
        const now = Date.now();
        if (now - lastHashTime > 300000) { // Every 5 minutes instead of 1 minute
          hashDocument();
    ...
    }, 30000); // Reduced to 30 seconds but only saves if there are unsaved changes

    return () => clearInterval(interval);
  }, [saveToLocalStorage, isAuthenticated, localDocumentId, lastHashTime, tiptapContent, editorContent, unsavedChanges]);
